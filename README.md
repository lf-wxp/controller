...
> **核心亮点**：协议逻辑（encode / decode / CRC / HMAC / replay window）在三处共享同一份 Rust 源码，
...
| 特性 | 说明 |
|------|------|
| **Frame** (21B, magic 0xC71E) | 手柄→Host 广播摇杆状态，CRC-16 校验、无 HMAC 签名（只读） |
...
| 任务 | 源文件 | 职责 |
|------|--------|------|
| `esp_now_broadcast_task` | [`src/transport/esp_now/mod.rs`](src/transport/esp_now/mod.rs) | ESP-NOW 广播 Frame（约 30 Hz） |
| `esp_now_receive_task` | 同上 | 接收 Command 帧并交给 `dispatch_command` |
...
| [`crates/examples/controller-receiver-demo/`](../crates/examples/controller-receiver-demo/) | 无硬件验证者 | 纯 host 侧 Frame 编解码 + seq gap 检测 demo（CI 跑） |
