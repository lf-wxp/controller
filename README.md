...
> 配合 [`peer_registry`](src/peer_registry.rs) + `Announce`/`AnnounceReply`/`AssignId`
...
│   ├── peer_registry.rs          # 已发现接收方目录（mac↔receiver_id，见 dest_mask）
...
| **AnnounceReply** (Response) | 接收方回报自身 mac + role_tag + rssi_dbm，供手柄侧 [`peer_registry`](src/peer_registry.rs) 入库 |
...
| 拨动开关 | IO15 | ⚠️ strapping，需 Pull::Up；同时驱动 4 颗彩灯 |
...
| `esp_now_receive_task` | 同上 | 接收 Command / Response 帧；`AnnounceReply` 交 [`peer_registry::upsert`](src/peer_registry.rs) 入库 |
...
| `led_effects_task` | [`src/hal/led_effects.rs`](src/hal/led_effects.rs) | LED 特效队列（闪烁 / 心跳） |
...
```text
[1] 长按 Switch 800ms  ─────► 进入 Selecting 模式
...
- Peer 目录：[`src/peer_registry.rs`](src/peer_registry.rs)（`heapless::Vec<PeerEntry, 32>`，无堆分配）
...
行 4 (y=40) : Btn [1][2][3][4][J][S]   （按下按钮反色方块）
