...
- [ESP-NOW 接收端参考实现](#esp-now-接收端参考实现)
  ...
  - [关键点说明](#关键点说明)
    ...
    - [3. 空气中的干扰帧](#3-空气中的干扰帧)
    - [4. 延迟表现](#4-延迟表现)
    - [5. 1 对 N 广播场景](#5-1-对-n-广播场景)
    - [6. 与 BLE 双轨并存](#6-与-ble-双轨并存)
  - [烧录 \& 验证](#烧录--验证)
...
- **状态帧无鉴权**：单向广播 21B 明文帧，接收端零密钥零配置即可解码；
...
async fn main(_spawner: Spawner) -> ! {
    ...
    info!("[RX] Booting ESP-NOW receiver");
    ...
    loop {
        ...
        match decode_frame(data) {
            Ok(frame) => {
                ...
                let s = &frame.payload;
                info!(
                    "[RX] seq={} joy=({},{}) knob=({},{}) btn=0b{=u8:08b} sw={}",
                    frame.seq,
                    s.joy_x, s.joy_y,
                    s.knob_1, s.knob_2,
                    s.buttons,
                    s.button(ButtonBits::Switch1),
                );
            }
...
好在 `decode_frame` **只认 `FRAME_MAGIC = 0xC71E`**，其它 magic 以及外部干扰帧全部返回
...
- 如需 1 对 1 单播：把 `src/transport/esp_now/mod.rs` 里的 `BROADCAST_ADDRESS`
  改成目标 MAC，并在发送端 `add_peer(target_mac)`；接收端无需改动
...
2. 上电发送端手柄
3. 拨动摇杆/按下按钮 → 接收端每 33 ms 打印一行 `seq=... joy=(x,y) ...`
