...
本文面向 **下游接入方**（另一个 receiver 项目 / 控制端 / 网关等），说明如何以**不拷贝源码**的方式复用 `protocol`，并正确启用 feature、注入密钥、编写最小可运行示例。
...
`protocol` 已是**独立 crate**，无需拷贝源码。按项目场景任选一种：
...
# 下游项目的 Cargo.toml
[dependencies]
protocol = { git = "https://github.com/lf-wxp/controller", tag = "protocol-v0.2.0", default-features = false }
```
...
```toml
[dependencies]
protocol = { version = "0.1", default-features = false }
```
...
```toml
[dependencies]
protocol = { path = "../controller/crates/protocol", default-features = false }
```
...
```toml
[dependencies]
protocol = { path = "../protocol", default-features = false }
```
...
```rust
use protocol::{DecodeError, FRAME_LEN, Frame, GamepadState, decode_frame};
...
```rust
use protocol::{Frame, GamepadState};
